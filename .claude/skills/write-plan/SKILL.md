...
name: write-plan
description: >-
  タスク計画を .artifacts/features/<feature>/PLANS.md へ書き込む時に使用する。
...
タスク計画を `.artifacts/features/<feature>/PLANS.md` へ書き込む手順を定義する。

<workflow>

1. `Read` で `./template.md` を読み込む。
1. 既存の `.artifacts/features/<feature>/PLANS.md` があれば読み込み、更新箇所を特定する。
...
- **責務を分ける**: PLANS.md はタスクの全体像と依存グラフを示すものとし、各タスクの詳細仕様は
  refiner が `TASK_<phase>_<n>.md` に展開する。
- **テンプレート構造を保つ**: テンプレートで定義されたセクションを追加・削除・並べ替えしない。
- **ファイル名と ID を対応させる**: タスク ID は後段の `TASK_<phase>_<n>.md` のファイル名と
