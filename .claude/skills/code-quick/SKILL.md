...
前工程のドキュメント（`TASK_<phase>_<n>.md`・`SPECIFICATION.md`・`ARCHITECTURE.md` 等）は存在しない前提とし、
...
実装した変更に対するテストと静的解析を実行し、満たされるまで修正する。
Python では次を用いる。

- テスト: `uv run pytest <対象>`
...
成果物のドキュメント化（SUMMARY 追記・STATE 記録・タスクファイル更新）は**行わない**。
...
- **指示の範囲に留まる**: 渡された指示を仕様とみなし、その範囲内で変更する。指示外の機能追加や
  広範なリファクタはしない。
- **ドキュメントを参照しない・作成しない**: 前工程の仕様書・タスク仕様を前提にせず、SUMMARY.md・
  STATE.md・タスクファイル等の成果物ドキュメントも生成・更新しない。
