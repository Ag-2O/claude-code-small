...
description: >-
  コードレビュー: $ARGUMENTS
  STATE.md で done のタスクを reviewer サブエージェントで一括並列にレビューし、指摘を review-fix タスクへ起票する。
...
- **引数 1 個（`<feature>`）**: 一括レビューモード。STATE.md で `done` のタスクをすべて `reviewer` で並列にレビューする。
...
1. `.claude/scripts/state/state_tool.py show --feature <feature>` で STATE.md を読み、
   状態が `done` のタスク ID（`TASK_<phase_num>_<task_num>` 形式）を抽出する。STATE.md が無ければ
...
1. **レビュー対象タスクすべてに対して、`reviewer` サブエージェントを単一メッセージ内で並列起動する**。
   ...
   `TASK_<phase>_<n>.md`・SUMMARY.md・SPECIFICATION.md の内容を prompt へ複製しない（reviewer が自身で `Read` する）。
...
- **完了タスクのみレビューする**: STATE.md で `done` のタスクだけが対象。`in_progress` や `blocked` は
  実装が確定していないため対象外。
