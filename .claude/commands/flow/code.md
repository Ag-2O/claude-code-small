...
description: >-
  実装: $ARGUMENTS
  plan.md の Wave 順に未完了タスクを coder サブエージェントで TDD 実装し、完了を state.db へ記録する。
...
- **引数 1 個（`<feature>`）**: Wave モード。plan.md の Wave 順に未完了タスクを `coder` で並列実行する。
...
1. `.artifacts/features/<feature>/plan.md` を `Read` で読み、「Wave（並列実行グループ）」セクションから
   Wave とその構成タスク ID を抽出する。`plan.md` が存在しない場合は実行を中止し、
