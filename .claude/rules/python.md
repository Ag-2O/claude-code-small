...
## 7. フォーマットと lint の対応（必須）

- **フォーマットはフックが自動実行する**: Python ファイルを Write / Edit すると、PostToolUse フックが
  `ruff format` を自動適用する。エージェントが手で整形する必要はない。
- **lint はゲートで実行する**: `ruff check` はフックでは実行しない（編集ごとの lint は診断がコンテキストを
  圧迫するため）。ワークフローの検証ゲートでまとめて実行する。具体的なコマンドと手順は
  `base-python` スキルの「format と lint の実行」を参照する。
- **完了前に lint 違反を 0 件にする**: 実装・修正の完了報告前に lint ゲートを実行し、`select = ["ALL"]` の
  違反を解消する。問題 0 件を明示するか、残件がある場合はその理由を報告に含める。
